...
const (
	...
	// SubRoleLabelKey records the sub-role assigned to an LWS replica group.
	// It is controller-owned and applied dynamically to every Pod in the group.
	...
	// InitialSubRoleReplicasAnnotationKey stores the initial sub-role distribution
	// at rollout start as a JSON object whose keys are sub-role names.
...
// DisaggregatedSubRoleSpec defines an independently scalable routing partition
// within a configuration-identical parent role.
...
type DisaggregatedSubRoleSpec struct {
	...
	// when scaling is Static or omitted. The controller defaults it to 1.
	// +optional
...
type DisaggregatedRoleSpec struct {
	...
	// replica target is the sum of the sub-role targets.
	// +optional
	// +listType=map
	// +listMapKey=name
...
// SubRoleStatus defines the observed state of one sub-role.
...
type RoleStatus struct {
	...
	// SubRoleStatuses contains observed counts for this role's sub-roles.
	// +listType=map
	// +listMapKey=name
	// +optional
...
const (
	...
	// DisaggregatedSetSubRolesAssigned means every live group belonging to a
