...
And a new interface called `PodGroupProvider` will be added to manage PodGroup resources. Whenever a leader pod is created, 
...
- `SetPodMeta`: This method will be called by pod webhook to set the PodGroup affiliation for leader pods and worker pods, 
  ...
  Therefore, `SetPodMeta` directly passes in the pod object and sets labels and annotations for this pod

The interface called `ReplicaResourceProvider` is the composite interface of `BaseResourceProvider` and `PodGroupProvider`, 
...
and don't need to change the `ReplicaResourceProvider` interface or `PodGroupProvider` interface. 
...
1. Pod Webhook (`SetPodMeta`)

Generally, SetPodMeta only involves setting annotations or labels for Leader/Worker pods and does not involve actual API calls, so errors will not occur.
But the interface design still retains the ability to return an error. If there is indeed a kind of implementation of `SetPodMeta` needs to return an error, 
