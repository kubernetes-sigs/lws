...
We also need to change `updateConditions` func to return `allUpdateDone` which means all replicas have been updated and completed, not `updateDone`. The adjustment is made to clean up other revisions when all replicas have been updated, rather than immediately upon completion of the partition update.
```go
// old code
if updateDone {
