...
This KEP aims to allow resizing of workers by introducing a new field that defines the type of resize strategy. The current strategy will become `None` to keep the same behavior. `InPlace` and `RollingRestart` will be added. The `spec.leaderWorkerTemplate.size` field will become editable by relaxing the validating webhook.
...
const ResizePolicyInplace ResizePolicyType = "InPlace"
const ResizePolicyRestart ResizePolicyType = "Restart"
...
type LeaderWorkerTemplate struct {
    ...
    // InPlace updates the size without affecting already existing pods.
    // Restart updates the size and restarts existing pods.
	// +kubebuilder:default=None
	// +kubebuilder:validation:Enum={None,InPlace,Restart}
	// +optional
...
If the policy is `InPlace`, the underlying leader pod and statefulset will be resized accordingly.
For the `Restart` policy, a new annotation `leaderworkerset.sigs.k8s.io/size` describing the targeted size will be added to the `StatefulSets` pod template to trigger a rollout of the worker pods.
