...
func (r *LeaderWorkerSetReconciler) reconcileCombinedRollout(ctx context.Context, lws *leaderworkerset.LeaderWorkerSet, sts *appsv1.StatefulSet, revision string, templateChanged bool) (bool, ctrl.Result, error) {
	...
	if len(plan.deletes) > 0 {
		if err := r.prepareCombinedActuation(ctx, lws, sts, *sts.Spec.Replicas, revision, true); err != nil {
			return true, wait, err
		}
	}
...
// Preserve upstream prerequisites without refreshing away the snapshot on which
// reservations were planned. Both pruning and scheduling status may write LWS;
// any such write must make this pass conflict and retry with fresh observations.
func (r *LeaderWorkerSetReconciler) prepareCombinedActuation(ctx context.Context, lws *leaderworkerset.LeaderWorkerSet, sts *appsv1.StatefulSet, replicas int32, revision string, schedule bool) error {
	if err := r.combinedFence(ctx, lws, sts); err != nil {
		return err
	}
	...
	if schedule {
		// Status().Update mutates its argument's ResourceVersion. Keep the
		// original observation unchanged so the final fence detects that write.
		if err := r.reconcileWorkloadScheduling(ctx, lws.DeepCopy(), replicas, revision); err != nil {
			return err
		}
	}
	return r.combinedFence(ctx, lws, sts)
}
...
func (r *LeaderWorkerSetReconciler) applyCombinedStatefulSet(ctx context.Context, lws *leaderworkerset.LeaderWorkerSet, old, next *appsv1.StatefulSet, state string) error {
	...
	schedule := true
	if state != "" {
		decoded, err := combinedModelDecode(state)
		if err != nil {
			return err
		}
		// Freeze changes only the partition/state, retaining the actual old
		// template and replicas. Do not schedule the live spec under its old key.
		schedule = decoded.Phase != combinedFreeze
	}
	if err := r.prepareCombinedActuation(ctx, lws, old, *next.Spec.Replicas, revisionutils.GetRevisionKey(next), schedule); err != nil {
		return err
	}
...
func (r *LeaderWorkerSetReconciler) publishCombinedStatefulSet(ctx context.Context, lws *leaderworkerset.LeaderWorkerSet, sts *appsv1.StatefulSet, state combinedModelState) error {
	...
	if err := r.prepareCombinedActuation(ctx, lws, sts, *sts.Spec.Replicas, state.Revision, true); err != nil {
		return err
	}
