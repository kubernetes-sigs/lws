...
// or sub-role and garbage-collects obsolete scalers. It also writes back
// status.replicas / status.selector / conditions so HPA sees a valid /scale target.
...
func ScalerName(dsName string, key RoleKey) string {
...
func (m *ScalerManager) Reconcile(
	...
	seedFor func(key RoleKey, existing ScalerMap) int32,
) (ScalerMap, error) {
	log := logf.FromContext(ctx)

	externalRoles := make(map[RoleKey]bool)
...
func (m *ScalerManager) create(ctx context.Context, ds *disaggregatedsetv1.DisaggregatedSet, key RoleKey, seed int32) (*disaggregatedsetv1.DisaggregatedSetRoleScaler, error) {
...
func (m *ScalerManager) WriteStatus(
	...
	ds *disaggregatedsetv1.DisaggregatedSet,
	scalers ScalerMap,
	observedReplicas map[RoleKey]int32,
) error {
