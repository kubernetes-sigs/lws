...
func TestLeaderStatefulSetApplyConfigNilChecks(t *testing.T) {
	t.Run("nil replicas and nil rolling update config should not panic", func(t *testing.T) {
		lws := &leaderworkerset.LeaderWorkerSet{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-sample",
				Namespace: "default",
		...
		stsApplyConfig, err := constructLeaderStatefulSetApplyConfiguration(lws, 0, 1, "revision-1")
		if err != nil {
			t.Fatalf("failed with error: %s", err.Error())
		}
		if stsApplyConfig == nil {
			t.Fatal("expected non-nil apply configuration")
		}
	})
}

func TestRollingUpdateParametersNilChecks(t *testing.T) {
	reconciler := &LeaderWorkerSetReconciler{}
	t.Run("nil replicas and nil rolling update config should not panic", func(t *testing.T) {
		lws := &leaderworkerset.LeaderWorkerSet{
			ObjectMeta: metav1.ObjectMeta{
		...
		partition, replicas, err := reconciler.rollingUpdateParameters(context.TODO(), lws, sts, "revision-1", false)
		if err != nil {
			t.Fatalf("failed with error: %s", err.Error())
		}
		if partition != 0 {
