...
// getMinResources calculates the minimum resources needed for the entire PodGroup [1 Leader + (size-1) Worker pods]
func (v *VolcanoProvider) getMinResources(lws *leaderworkerset.LeaderWorkerSet) corev1.ResourceList {
	...
	totalResources := resource.PodRequests(&corev1.Pod{Spec: leaderTemplate.Spec}, resource.PodResourcesOptions{})

	// Calculate and add worker resources for (size-1) workers.
	workerCount := ptr.Deref(lws.Spec.LeaderWorkerTemplate.Size, 1) - 1
	if workerCount > 0 {
		workerResources := resource.PodRequests(&corev1.Pod{Spec: lws.Spec.LeaderWorkerTemplate.WorkerTemplate.Spec}, resource.PodResourcesOptions{})
