...
// The defaultBaseResourceProvider is the default implementation for creating resources like HeadlessService, ResourceClaim, etc.
// Generally, in lws, there is only one implementation for creating these resources, and you can embed defaultBaseResourceProvider into the newly written replica resource provider,
// you can refer to the implementation in volcano_provider.go.
...
// When users do not want to create resources like podgroup for each replica, they can use defaultReplicaResourceProvider as the ReplicaResourceProvider
