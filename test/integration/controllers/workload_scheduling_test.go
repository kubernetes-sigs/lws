...
var _ = ginkgo.Describe("Workload-aware scheduling controller", func() {
	...
	ginkgo.It("creates replica Workload and PodGroups before releasing the leader StatefulSet", func() {
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lws), workload)).To(gomega.Succeed())
			...
			for i := range groups.Items {
				...
				g.Expect(groups.Items[i].Spec.WorkloadRef.WorkloadName).To(gomega.Equal(lws.Name))
	...
	ginkgo.It("updates the stable whole-LWS gang minimum when replicas scale", func() {
		...
		assertGangMinimum := func(g gomega.Gomega, want int32) {
			...
			g.Expect(k8sClient.Get(ctx, types.NamespacedName{Namespace: ns.Name, Name: schedulerprovider.GetLWSGroupName(lws.Name)}, group)).To(gomega.Succeed())
