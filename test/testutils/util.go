...
func CheckSizeAnnotation(pod corev1.Pod, size int) error {
	podAnnotationSize, exists := pod.ObjectMeta.Annotations[leaderworkerset.SizeAnnotationKey]
	if !exists {
		return errors.New("Pod size annotation not set")
	}
	podAnnotationSizeInt, err := strconv.Atoi(podAnnotationSize)
	if err != nil {
		return err
	}
	if podAnnotationSizeInt != size {
		return fmt.Errorf("Pod annotation value is not correct, want: %v, got: %v", size, podAnnotationSizeInt)
	}
	return nil
}
