...
func getPodGroupGVK(provider schedulerprovider.SchedulerProvider) schema.GroupVersionKind {
	switch provider.(type) {
	case *schedulerprovider.VolcanoProvider:
...
func validatePodGroupSpec(podGroup *unstructured.Unstructured, lws *leaderworkerset.LeaderWorkerSet) {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	gomega.Expect(found).To(gomega.BeTrue())
	gomega.Expect(minMember).To(gomega.Equal(int64(*lws.Spec.LeaderWorkerTemplate.Size)))
}
...
func ExpectValidPodGroups(ctx context.Context, k8sClient client.Client, provider schedulerprovider.SchedulerProvider, lws *leaderworkerset.LeaderWorkerSet, expectedCount int) {
	...
	gomega.Eventually(func() (int, error) {
		...
		return len(podGroupList.Items), nil
...
func UpdatePodGroupAtIndex(ctx context.Context, k8sClient client.Client, schedulerProvider schedulerprovider.SchedulerProvider, lws *leaderworkerset.LeaderWorkerSet, groupIndex string) {
	gvk := getPodGroupGVK(schedulerProvider)
	if gvk.Empty() {
		ginkgo.Skip("Unsupported scheduler provider for PodGroup update")
		return
	}
...
func ExpectValidPodGroupAtIndex(ctx context.Context, k8sClient client.Client, provider schedulerprovider.SchedulerProvider, lws *leaderworkerset.LeaderWorkerSet, groupIndex string) {
	gvk := getPodGroupGVK(provider)
	if gvk.Empty() {
		ginkgo.Skip("Unsupported scheduler provider for PodGroup validation")
		return
